...
Status: **PLAN ONLY.** Not started. Staged, HW-validated, no incremental commits mid-refactor (per [feedback_no_incremental_commits_during_refactor]). Fix BY HAND — no python/sed sweeps.
...
## 1. Why (the N×M → N+M argument)
Every interface consumes sensor data. If each sensor emits its own shape, each interface must know each sensor → (interfaces × sensors) integrations, and a *new* sensor means touching BLE + web + OLED + serial. A shared envelope collapses this: each sensor emits one shape, each interface parses one shape, and a new sensor shows up everywhere for free. The payoff is extensibility, not just tidiness.
...
|---|---|---|---|
| **imu** (bno055.cpp:586) | `valid` ✓ | `ageMs`+`timestamp` (dup) | rename `timestamp`→`ts`, `ageMs`→`age`. Lowest effort. |
| **rtc** (ds3231.cpp:385) | `valid` ✓ | `ts` ✓ | add `connected` (`gRtcConnected`), optional `age`. |
| **presence** (sths34pf80.cpp:479) | `valid` ✓ | `ts` ✓ | add `connected`, optional `age`. |
...
## 7. Buffer constraint (the real ceiling is the RX side)
`RemoteSensorData.jsonData` is **`char[256]`** (`REMOTE_SENSOR_BUFFER_SIZE`, System_ESPNow_Sensors.h) for ALL non-thermal sensors — the hard cap, *below* the generous send-side `gSensorSpecs` budgets (tof 1024, imu 512). The envelope adds ~4 short keys (~40-50 B). **Keep keys short** (`valid`/`connected`/`ts`/`age`), and for the tightest sensors (gps/input at 256 B) verify headroom — if it doesn't fit, drop `age` (derivable from `ts`) or emit it json-only on roomy transports (CLI/web), not on ESP-NOW.
...
## 10. Risks & rollback
- **FlutterApp parse** (§5) — update the app's keys in the same flash cycle (you control it; no shim). A coordinated app change, not a compat problem.
- **256 B ESP-NOW RX cap** (§7) — measure headroom on gps/input.
...
## 11. Checklist
- [ ] `sensorEnvelopeBegin` helper added
- [ ] Additive pass: gps / gamepad / apds / fmradio (+ upstream ts) / ano-dedup carry `valid`+`connected`+`ts`
- [ ] fmradio upstream: FMRadioCache timestamp + real dataValid
- [ ] Decide mic-level (cache a ts, or leave out)
- [ ] FlutterApp sensor parse updated for the new keys (same flash cycle; no shim)
- [ ] 256 B RX headroom verified on gps + input
- [ ] Build green (HW_BOARD=xiao_s3 + FeatherS3) + HW-validated (sensors json, per-sensor read, mesh remote read)
- [ ] Thermal summary-envelope extraction (optional)
