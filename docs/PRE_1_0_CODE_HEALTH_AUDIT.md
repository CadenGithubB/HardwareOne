...
- **[System_Camera_Video.cpp:514](components/hardwareone/System_Camera_Video.cpp#L514)** `M`, _per-request_ - Video list endpoint builds a String list then re-parses it, walking the dir twice
  - **PARTIALLY FIXED 2026-08-25.** The double walk is gone: `count` is now derived from the single `getVideoRecordingsList()` pass in handleVideoRecordingsList, and the same fix was applied to both branches of `cmd_cameravideolist`. `getVideoRecordingCount()` has been **deleted** so it cannot be called again. The String-grown-without-reserve churn described below is UNCHANGED and still open.
