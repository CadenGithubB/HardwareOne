...
class LootVisibilityTests(unittest.TestCase):
    ...
    def test_loot_depth_support_and_partial_chests(self) -> None:
        ...
        self.assertIn("LOOT_VISIBILITY_RESULT PASS 49", result.stdout.splitlines(), result.stdout + result.stderr)
        self.assertEqual(49, sum(line.startswith("PASS ") for line in result.stdout.splitlines()))
    ...
    def test_visibility_mutation_is_detected(self) -> None:
        ...
        self.assertNotIn("LOOT_VISIBILITY_RESULT PASS 49", result.stdout.splitlines())
