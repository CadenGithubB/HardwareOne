...
- Connect > Bluetooth > R1 Ring on the display: live heart rate, heart-rate variability, blood oxygen and battery from a paired ring, with Connect, Disconnect and an AutoConn toggle. Opening the screen asks the ring for readings straight away instead of waiting for it to volunteer them.
...
- Turning the network on or off is now admin-only: `openwifi`, `closewifi`, `wifidisconnect`, `radiopower`, `openhttp` and `closehttp`. So are the Bluetooth controls (`openble`, `closeble`, `bledisconnect`, `bleadv`, `bleautostart`, `blemode`, `bleautoconnect`), the wearable connect verbs (`openg2`, `closeg2`, `ringconnect`, `ringdisconnect`), and `openmqtt`/`closemqtt`. Read-only status commands stay open to any signed-in user. An automation owned by a non-admin that calls one of these will now fail, since automations run as their owner.
