...
// =============================================================================
// A tap-navigated user manager reached from the Apps launcher (the row appears
...
//     DETAIL "<- <name>" / Role: <role> > / Delete
//       ROLE   "<- Back" + [X] guest/user/admin (+superadmin if pairer is super)
//       DELETE "<- Cancel" / Confirm Delete <name>
//   ADD      keyboard: username -> password -> role pick -> useradd
//
// Core safety (rank ceiling, founder immutability) is enforced by the command
// layer; the UI surfaces the Error: text rather than re-implementing it.
...
// Render the user list. Called from the Apps launcher tap dispatch.
