...
inline void streamDashboardInner(httpd_req_t* req, const String& username) {
...
#if ENABLE_ANO_ENCODER
    "          'Seesaw':{key:'input',label:'ANO Encoder (Seesaw)',compiled:'inputCompiled'},\n"
#else
    "          'Seesaw':{key:'input',label:'Gamepad (Seesaw)',compiled:'inputCompiled'},\n"
#endif
 ...
 #if ENABLE_ANO_ENCODER
  httpd_resp_send_chunk(req, "<script>", HTTPD_RESP_USE_STRLEN);
  streamAnoEncoderDashboardDef(req);
  httpd_resp_send_chunk(req, "</script>", HTTPD_RESP_USE_STRLEN);
 #endif
 #if ENABLE_GPS_SENSOR
  httpd_resp_send_chunk(req, "<script>", HTTPD_RESP_USE_STRLEN);
  ...
  httpd_resp_send_chunk(req,
    ...
    "<div style='font-weight:bold;margin-bottom:0.5rem;color:rgba(255,255,255,0.9)'>Date &amp; Time</div>"
    "<div class='sys-card-row'><span>Date:</span><strong id='sys-date'>--</strong></div>"
    "<div class='sys-card-row'><span>Time:</span><strong id='sys-time'>--</strong></div>"
      ...
      "window.Dash.updateSystem=function(d){"
        ...
        "try{"
          ...
          "if(c.llm){"
            ...
            "var mn=(ll.model&&ll.model.length>0)?ll.model.split('/').pop().replace(/\\.bin$/i,''):'None';"
            "window.Dash.setText('conn-llm-model',mn);"
  ...
  httpd_resp_send_chunk(req,
    ...
    "hw.postJSON('/api/user/settings',patch)"
    ...
    "hw.postFormText('/api/cli',{cmd:'dashboard layout '+desc})"
    ".catch(function(){});"
    ...
    "hw.postFormText('/api/cli',{cmd:'dashboard layout reset '+getGridTitle(_editingGridId)})"
    ".catch(function(){});"
    "window.Dash.closeLayoutEditor();"
    "location.reload();"
    "};"
    ...
    "return hw.fetchJSON('/api/user/settings')"
    ".then(function(j){"
