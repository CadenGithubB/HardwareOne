...
inline void streamAviPlayerJs(httpd_req_t* req) {
  httpd_resp_send_chunk(req,
    ...
    // Only the video stream's rate/scale describes the frame clock. Without the
    // 'vids' check the last strl wins, so an audio stream (scale=1, rate=16000)
    // sets fps to the sample rate.
    "          if (sid === 'strh' && readFourCC(dv,sp+8) === 'vids') {\n"
    ...
    // Skip chunks we don't decode (e.g. '01wb' audio) instead of stopping at the
    // first one. The bounds check still stops a truncated trailing chunk, and
    // adv >= 8 guarantees fp advances, so a garbage tail terminates.
    "            if (fp+8+fsize > buf.byteLength) break;\n"
    // A zero-length '00dc' is a dropped-frame placeholder: it holds a real time
    // slot, so it must be kept as a frame entry rather than ending the scan.
    "            if (fid === '00dc' || fid === '00db') frames.push({offset: fp+8, size: fsize});\n"
    ...
    "    }).catch(function(){});\n"
    "    var sc = hw.$('avi-scrub'); if (sc) sc.value = idx;\n"
    "    var tm = hw.$('avi-time');\n"
    "    if (tm) {\n"
    ...
    "    var pb = hw.$('avi-play'); hw.setText(pb, 'Play');\n"
    "  }\n"
    ...
    "    var pb = hw.$('avi-play'); hw.setText(pb, 'Pause');\n"
    ...
    "    var modal = hw.$('avi-modal');\n"
    "    var status = hw.$('avi-status');\n"
    "    var title = hw.$('avi-title');\n"
    "    var scrub = hw.$('avi-scrub');\n"
    "    if (!modal) return;\n"
    "    _aviStop();\n"
    ...
    "    hw.setText(title, base);\n"
    "    hw.setText(status, 'Loading…');\n"
    ...
    "        hw.setText(status, parsed.frames.length + ' frames @ ' + _aviState.fps.toFixed(1) + ' fps · ' + (parsed.meta.width||'?') + 'x' + (parsed.meta.height||'?'));\n"
    "        _aviDraw(0);\n"
    "      }).catch(function(e){ hw.setText(status, 'Error: ' + e.message); });\n"
    "  };\n"
    "  document.addEventListener('DOMContentLoaded', function(){\n"
    "    var modal = hw.$('avi-modal');\n"
    "    var closeBtn = hw.$('avi-close');\n"
    "    var playBtn = hw.$('avi-play');\n"
    "    var scrub = hw.$('avi-scrub');\n"
    "    if (closeBtn) closeBtn.onclick = function(){ _aviStop(); hw.hide(modal); };\n"
