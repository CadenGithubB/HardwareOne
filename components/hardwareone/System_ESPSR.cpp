...
static bool srSnipInit() {
  ...
  BaseType_t ret = xTaskCreatePinnedToCore(srSnipWriterTask, "sr_snip_wr", SR_SNIP_WRITER_STACK_WORDS, nullptr, 3, &gSrSnipWriterTask, 0);
  if (ret != pdPASS) {
...
bool startESPSR() {
  ...
  WARN_SYSTEMF("[SR_START] Step 4: Creating srTask (stack=%u, priority=%d, core=1)",
               (unsigned)SR_TASK_STACK_WORDS, (int)SR_TASK_PRIORITY_LEVEL);
