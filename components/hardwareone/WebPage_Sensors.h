...
<div class='sensors-container'>
  ...
  httpd_resp_send_chunk(req,
    ...
    "          if(isAno){\n"
    "            host.innerHTML='<div style=\"display:flex;gap:16px;align-items:center;margin-top:10px;flex-wrap:wrap\"><canvas id=\"'+id+'-dial\" width=\"90\" height=\"90\" style=\"border:1px solid var(--border);border-radius:50%;background:var(--crumb-bg)\"></canvas><div id=\"'+id+'-btns\" style=\"display:flex;flex-wrap:wrap;max-width:170px\"></div></div>';\n"
    "          }else{\n"
    ...
    "          }\n"
    "        }\n"
    "        if(isAno){ if(typeof window.hwRenderRemoteInput==='function') window.hwRenderRemoteInput(id,payload); }\n"
