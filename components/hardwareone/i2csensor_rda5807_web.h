    ...
    <div class='sensor-card' id='sensor-card-fmradio'>
      ...
      <div class='sensor-data' id='fmradio-data'>
        ...
        <div id='fmradio-controls' style='margin-top:12px;display:flex;gap:8px;flex-wrap:wrap'>
          ...
          <button class='btn btn-small' onclick="sendCmd('fmradiovolume ' + Math.max(0, parseInt(document.getElementById('fmradio-volume').innerText) - 1))">🔉 Vol-</button>
          <button class='btn btn-small' onclick="sendCmd('fmradiovolume ' + Math.min(15, parseInt(document.getElementById('fmradio-volume').innerText) + 1))">🔊 Vol+</button>
        </div>
